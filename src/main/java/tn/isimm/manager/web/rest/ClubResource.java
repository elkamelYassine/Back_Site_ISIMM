...
/**
 * REST controller for managing {@link tn.isimm.manager.domain.Club}.
 */
@RestController
    ...
    public ResponseEntity<Club> partialUpdateClub(@PathVariable(value = "id", required = false) final Long id, @RequestBody Club club)
        ...
        Optional<Club> result = clubRepository
            ...
            .map(existingClub -> {
                ...
                if (club.getPageFB() != null) {
                    existingClub.setPageFB(club.getPageFB());
                }
                if (club.getPageIg() != null) {
                    existingClub.setPageIg(club.getPageIg());
                }
